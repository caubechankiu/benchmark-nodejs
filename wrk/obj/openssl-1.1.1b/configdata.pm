...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=/Users/quangthinh/Documents/Agiletech/YAAPP/benchmark-nodejs/wrk/obj no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dtls no-dtls1 no-dtls1_2 no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-idea no-md2 no-msan no-psk no-rc5 no-sctp no-shared no-srp no-ssl-trace no-ssl3 no-ssl3-method no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "darwin-thread-multi-2level",
  perl_cmd => "perl",
  perl_version => "5.18.2",
  perlargv => [ "no-shared", "no-psk", "no-srp", "no-dtls", "no-idea", "--prefix=/Users/quangthinh/Documents/Agiletech/YAAPP/benchmark-nodejs/wrk/obj", "darwin64-x86_64-cc" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
  ...
  prefix => "/Users/quangthinh/Documents/Agiletech/YAAPP/benchmark-nodejs/wrk/obj",
  processor => "",
